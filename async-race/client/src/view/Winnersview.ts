import { winners, wins } from "../Store/store";
import "../styles/winnerStyle.css";

export const winnerspag = () => {
  const winnersPage = `
    <div class="winner">
        <div class="buttons">
  ...
  document.body.innerHTML = winnersPage;
};

const carImage = (color: string) => {
  return `<div class="carImg"><svg
    fill="${color}"
    version="1.1"
    id="Capa_1"
...
function renderWinnersTable() {
  const startIndex = (winners.pageNumber - 1) * winners.items;
  const endIndex = startIndex + winners.items;

  const winnersTableBody = document.querySelector(".winner tbody");
  winnersTableBody.innerHTML = "";
  wins.slice(startIndex, endIndex).forEach((winner) => {
    const row = document.createElement("tr");
    row.innerHTML = `
         <td class="id">${winner.finishedCar.id}</td>
         <td>${winner.finishedCar.name}</td>
         <td>${carImage(winner.finishedCar.color)}</td>
         <td class="time">${winner.timeAnim.toFixed(3)}</td>
         <td class="wins">${Math.round(winner.winsCount)}</td>
       `;
    winnersTableBody.appendChild(row);
  });
}

function sortTable(columnClass: string, ascending: boolean) {
  const tableRows = Array.from(document.querySelectorAll(".winner tbody tr"));

  tableRows.sort((a, b) => {
    const valueA = parseInt(a.querySelector(`.${columnClass}`).textContent);
    const valueB = parseInt(b.querySelector(`.${columnClass}`).textContent);
    return ascending ? valueA - valueB : valueB - valueA;
  });

  const tableBody = document.querySelector(".winner tbody");
  tableBody.innerHTML = "";
...
function attachEventListeners() {
  const idColumnHeader = document.querySelector(".winner th:first-child");
  const timeColumnHeader = document.querySelector(".winner th:nth-child(4)");
  const winsColumnHeader = document.querySelector(".winner th:nth-child(5)");
  ...
  idColumnHeader.addEventListener("click", () => {
    sortTable("id", ascendingId);
    ascendingId = !ascendingId;
  });

  timeColumnHeader.addEventListener("click", () => {
    sortTable("time", ascendingTime);
    ascendingTime = !ascendingTime;
  });

  winsColumnHeader.addEventListener("click", () => {
    sortTable("wins", ascendingWins);
